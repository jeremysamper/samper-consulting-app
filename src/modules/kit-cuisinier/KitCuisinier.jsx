...
function navigateToLinkedPage(page) {
  navigateToPage(page);
}
...
const KitCuisinier = ({ user, etablissement }) => {
  ...
  React.useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const [its, sps, rcs, dcs] = await Promise.all([
          legacySB.db.listKitItems(etabId),
          legacySB.db.listSops(etabId).catch(() => []),
          legacySB.db.listRecettes(etabId).catch(() => []),
          legacySB.db.listDocuments(etabId).catch(() => []),
        ]);
        if (mounted) {
          setItems(its);
          setSops(sps);
    ...
    const u1 = legacySB.realtime.subscribe('kit_items', refresh);
              ...
              {linkedDestPage && (
                <button
                  ...
                  onClick={() => {
                    ...
                    navigateToLinkedPage(linkedDestPage);
                  }}
                >
                  Ouvrir →
                </button>
...
const KitItemEditor = ({ item, etabId, sops, recettes, docs, onBack }) => {
  ...
  const save = async () => {
    if (!data.title?.trim()) { alertLegacy('Titre obligatoire.'); return; }
    if (data.linkedType && !data.linkedId) {
      alertLegacy('Sélectionnez l\'élément à lier ou choisissez "Aucun" comme type de lien.');
      return;
    }
    setSaving(true);
    try {
      if (!legacySB?.db?.upsertKitItem) throw new Error('Supabase non configure.');
      await legacySB.db.upsertKitItem({ ...data, etablissementId: etabId });
      onBack();
    } catch (err) {
  ...
  const deleteItem = async () => {
    if (!data.id) return;
    if (!confirmLegacy(`Supprimer la fiche "${data.title}" ?`)) return;
    try {
      if (!legacySB?.db?.deleteKitItem) throw new Error('Supabase non configure.');
      await legacySB.db.deleteKitItem(data.id);
      onBack();
