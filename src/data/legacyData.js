...
if (readText('sc_data_version', '') !== DATA_VERSION) {
  ...
  writeText('sc_data_version', DATA_VERSION);
}

export const scRead = (key, fallback) => readJson(key, fallback);
export const scWrite = (key, value) => writeJson(key, value);
...
export function useSupabaseList(tableName, etabId, listFn, mapFn) {
  const legacySB = dbService.getBridge();
  ...
  React.useEffect(() => {
    ...
    unsub = legacySB.realtime.subscribe(tableName, (payload) => {
      if (!mounted) return;
