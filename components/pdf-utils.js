...
window.PDFUtils = {
  ...
  _getHeaderHTML(title, etablissement) {
    ...
    const logo = etablissement?.logo_url
      || (() => { try { return JSON.parse(localStorage.getItem('sc_app_logo') || 'null'); } catch { return null; } })();
  ...
  _getCurrentEtablissement() {
    ...
    try {
      const etabIdRaw = localStorage.getItem('sc_current_etab'); // legacy fallback
      if (etabIdRaw) {
        const etabId = JSON.parse(etabIdRaw);
        const found = DEMO_DATA.etablissements.find(e => e.id === etabId);
        if (found) return found;
      }
    } catch {}
    return DEMO_DATA.etablissements[0];
  },
  ...
  printElement(elementId, title = 'Document', options = {}) {
    ...
    if (!element) {
      if (window.notify) window.notify('Zone à imprimer introuvable : ' + elementId, 'error');
      else alert('Zone à imprimer introuvable : ' + elementId);
      return;
    }
  ...
  async exportElementToPdf(elementId, fileName = 'document.pdf', options = {}) {
    const element = document.getElementById(elementId);
    if (!element) { alert('Zone à exporter introuvable : ' + elementId); return; }
