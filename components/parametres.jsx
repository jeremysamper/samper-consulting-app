...
const Parametres = ({ user }) => {
  const [etablissements, setEtablissements] = React.useState(DEMO_DATA.etablissements || []);
  ...
  const save = async (etab) => {
    if (!canEdit) {
      alert("Vous n'avez pas les droits pour modifier les établissements (rôle consultant requis).");
      return;
    }
    ...
    if (window.SB) {
      try {
        const saved = await window.SB.db.upsertEtablissement(payload);
        if (!saved || !saved.id) {
          window.notify("Échec silencieux de la sauvegarde (vérifiez les permissions RLS sur Supabase).", 'error');
          return;
        }
        ...
        window.notify("Erreur enregistrement : " + err.message + (err.code === '42501' ? "\n\n(Erreur de permissions RLS — vérifiez votre rôle)" : ''), 'error');
        return;
      }
    } else {
  ...
  const confirmDelete = async (id) => {
    ...
    if (window.SB) {
      try {
        await window.SB.db.deleteEtablissement(id);
        ...
        <div style={{ padding:'16px 20px', display:'flex', flexDirection:'column', gap:14 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div style={ps.field}>
              ...
              <div style={ps.readField}>{DEMO_DATA.roles[user.role]?.label}</div>
            </div>
            <div style={ps.field}>
