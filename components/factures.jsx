...
const Factures = ({ user, etablissement }) => {
  ...
  const [form, setForm] = React.useState({
    numero: generateFactureNumber(), // Placeholder synchrone (fallback localStorage)
  ...
  React.useEffect(() => {
    ...
    if (!window.SB || !window.SB.db.getNextFactureNumber) return;
    numeroLoadedRef.current = true;
    (async () => {
      try {
        const numero = await window.SB.db.getNextFactureNumber(etabId, todayStr);
        setForm(prev => ({ ...prev, numero }));
      } catch (err) {
        console.warn('[Factures] getNextFactureNumber failed', err);
        // On garde le placeholder localStorage déjà en place
      }
    })();
  }, [etabId]);
  ...
  React.useEffect(() => {
    if (!window.SB) {
      setEtabsAll(DEMO_DATA.etablissements || []);
      return;
    }
    let mounted = true;
    (async () => {
  ...
  const [emailEnabled, setEmailEnabled] = React.useState(() => {
    if (window.SB?.db?.getUserSettingSync) {
      const v = window.SB.db.getUserSettingSync('email_facture_enabled');
      if (v != null) return !!v;
    }
    // Fallback localStorage legacy
    try {
      const raw = localStorage.getItem('sc_email_facture_enabled');
  ...
  const [emailTemplate, setEmailTemplate] = React.useState(() => {
    if (window.SB?.db?.getUserSettingSync) {
      const v = window.SB.db.getUserSettingSync('email_facture_template');
      if (v) return String(v);
    }
    try {
      const raw = localStorage.getItem('sc_email_facture_template');
      if (raw) return raw;
    } catch(e) {}
  ...
  React.useEffect(() => {
    if (!window.SB?.db?.setUserSetting) return;
    // Si la valeur est dans le cache, rien à migrer
    const cachedEnabled = window.SB.db.getUserSettingSync?.('email_facture_enabled');
    const cachedTemplate = window.SB.db.getUserSettingSync?.('email_facture_template');
    (async () => {
      try {
        if (cachedEnabled == null) {
          const local = (() => { try { return localStorage.getItem('sc_email_facture_enabled'); } catch { return null; } })();
          if (local != null) {
            await window.SB.db.setUserSetting('email_facture_enabled', local !== 'false');
            try { localStorage.removeItem('sc_email_facture_enabled'); } catch(e) {}
          }
        }
        if (cachedTemplate == null) {
          const local = (() => { try { return localStorage.getItem('sc_email_facture_template'); } catch { return null; } })();
          if (local) {
            await window.SB.db.setUserSetting('email_facture_template', local);
            try { localStorage.removeItem('sc_email_facture_template'); } catch(e) {}
          }
        }
  ...
  React.useEffect(() => {
    if (!window.SB?.db?.setUserSetting) return;
    window.SB.db.setUserSetting('email_facture_enabled', !!emailEnabled).catch(err => {
      console.warn('[Factures] save email_facture_enabled failed', err);
    });
  }, [emailEnabled]);
  React.useEffect(() => {
    if (!window.SB?.db?.setUserSetting) return;
    window.SB.db.setUserSetting('email_facture_template', String(emailTemplate || '')).catch(err => {
  ...
  const generatePDF = async (sendToDocs) => {
    ...
    try {
      ...
      window.notify('Erreur génération facture : ' + err.message, 'error');
    } finally {
      setBusy(false);
    }
  };
  ...
  const sendFactureToDocuments = async (fileName) => {
    if (!window.SB) throw new Error('Supabase non configuré');
    ...
    if (!yearFolder) {
      yearFolder = await window.SB.db.createFolder({
    ...
    if (!monthFolderObj) {
      monthFolderObj = await window.SB.db.createFolder({
    ...
    const docs = await window.SB.db.listDocuments(targetEtabId);
  ...
  // On commence par un placeholder synchrone (fallback localStorage) puis
  // on remplace par le vrai numéro DB dès qu'il revient — comme au mount.
  const newFacture = async () => {
    // Étape 1 : reset immédiat avec placeholder local (UX réactif)
    setForm({
    ...
    if (window.SB && window.SB.db.getNextFactureNumber) {
      try {
        const numero = await window.SB.db.getNextFactureNumber(etabId, todayStr);
        setForm(prev => ({ ...prev, numero }));
      } catch (err) {
          ...
          <div style={{ ...fac.modal, width: 600, maxWidth: '94vw', maxHeight: '92vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={fac.modalFooter}>
                ...
                onClick={() => {
                  ...
                  if (navigator.clipboard && navigator.clipboard.writeText) {
                    navigator.clipboard.writeText(txt)
                      .then(() => window.notify('✓ Email copié dans le presse-papiers. Collez-le dans votre client mail.', 'success'))
                      .catch(() => window.notify('Impossible de copier. Sélectionnez le texte manuellement.', 'error'));
                  } else {
                    window.notify('Presse-papiers non disponible. Sélectionnez le texte manuellement.', 'warning');
                  }
                }}
              >
                📋 Copier
              ...
              <button
                ...
                onClick={() => {
                  if (!emailDraft.to) {
                    if (!window.confirm('Aucune adresse email. Ouvrir quand même le client mail (vous pourrez la renseigner dedans) ?')) return;
                  }
                  ...
                  if (mailtoUrl.length > 2000) {
                    if (!window.confirm('Le message est très long et certains clients mail risquent de le tronquer. Continuer (recommandé : utiliser "Copier" à la place) ?')) return;
                  }
          ...
          <div style={{ ...fac.modal, width: 580, maxWidth: '94vw' }}>
            ...
            <div style={fac.modalFooter}>
              <button
                ...
                onClick={() => {
                  if (window.confirm('Restaurer le template par défaut ?')) {
      ...
      <table style={fr.detailTable}>
        <tbody>
          ...
          <tr style={{ background: '#f5f0e1' }}>
            ...
            <td style={{ ...fr.detailVal, fontSize: 14, fontWeight: 700, color: '#92702A' }}>
...
function generateFactureNumber() {
  ...
  // Compteur séquentiel basé sur localStorage (juste pour avoir un n° unique par jour)
  const key = `fac_counter_${y}${m}${dd}`;
  let cnt = parseInt(localStorage.getItem(key) || '0') + 1;
  localStorage.setItem(key, String(cnt));
  return `FAC-${y}${m}${dd}-${String(cnt).padStart(2, '0')}`;
}
...
const fr = {
  ...
  titleBar: { textAlign: 'center', fontSize: 16, fontWeight: 700, padding: '10px 0', borderBottom: '2px solid #92702A', marginBottom: 18, letterSpacing: 0.5, color: '#92702A', fontFamily: 'Arial, Helvetica, sans-serif' },
  twoColTop: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 18 },
  partyBox: { padding: '10px 12px', background: '#fafaf6', borderLeft: '3px solid #92702A', borderRadius: 4 },
  partyLabel: { fontSize: 10, fontWeight: 700, color: '#92702A', textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 4, fontFamily: 'Arial, Helvetica, sans-serif' },
  ...
  sectionTitle: { fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, color: '#92702A', marginBottom: 6, paddingBottom: 4, borderBottom: '1px solid #d4c8a0', fontFamily: 'Arial, Helvetica, sans-serif' },
