...
const SOP = ({ user, etablissement }) => {
  ...
  React.useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const [sopsList, execList] = await Promise.all([
          window.SB.db.listSops(etabId),
          window.SB.db.listSopExecutions(etabId, { limit: 100 }),
        ]);
        if (mounted) {
          setSops(sopsList);
    ...
    const u1 = window.SB.realtime.subscribe('sops', refresh);
    const u2 = window.SB.realtime.subscribe('sop_executions', refresh);
    ...
    <div style={ss.root}>
      ...
      {activeTab === 'liste' ? (
        <SopList
          ...
          onStart={async (sop) => {
            try {
              const exec = await window.SB.db.startSopExecution({
...
const SopChecklist = ({ execution, sop, user, etablissement, onBack }) => {
  ...
  React.useEffect(() => {
    if (!window.SB || !execution) return;
    window.SB.db.listSopStepStates(execution.id).then(states => {
      const map = {};
  ...
  const toggleStep = async (sectionId, stepId) => {
    ...
    if (window.SB) {
      try {
        await window.SB.db.toggleSopStep({
  ...
  const finish = async (statut) => {
    if (busy) return;
    if (statut === 'abandonnee' && !window.confirm('Abandonner cette exécution ? Les cases cochées seront conservées dans l\'historique.')) return;
    if (statut === 'terminee' && checkedSteps < totalSteps && !window.confirm(`${totalSteps - checkedSteps} étape(s) non cochée(s). Valider quand même ?`)) return;
    setBusy(true);
    try {
      await window.SB.db.finishSopExecution(execution.id, {
...
const SopHistory = ({ executions, sops, user, canManage }) => {
  ...
  const exportExecutionPDF = (exec) => {
    ...
    if (!sop) {
      window.notify('SOP introuvable pour cette exécution', 'error');
      return;
    }
    ...
    try {
      window.PDFUtils.printElement(tempId, sop.titre, { noBrandHeader: false });
    } catch (err) {
      window.notify('Erreur lors de la génération PDF', 'error');
...
const SopTemplatesModal = ({ etabId, existingSops, onClose }) => {
  const templates = window.SOP_TEMPLATES || [];
  ...
  const importSelected = async () => {
    ...
    setBusy(false);
    alert(`${count} SOP importée${count > 1 ? 's' : ''}.`);
    onClose();
  };
...
const SopEditor = ({ sop, etabId, onBack, onSaved }) => {
  ...
  const removeSection = (idx) => {
    if ((data.sections || []).length <= 1) { alert('Au moins une section.'); return; }
  ...
  const save = async () => {
    if (!data.titre?.trim()) { alert('Titre obligatoire.'); return; }
    const totalSteps = (data.sections || []).reduce((s, sec) => s + (sec.etapes || []).length, 0);
    if (totalSteps === 0) { alert('Ajoutez au moins une étape.'); return; }
    setSaving(true);
    try {
      const saved = await window.SB.db.upsertSop({ ...data, etablissementId: etabId });
      onSaved(saved);
    } catch (err) {
      window.notify('Erreur : ' + err.message, 'error');
      setSaving(false);
  ...
  const deleteSop = async () => {
    if (!data.id) return;
    if (!window.confirm(`Supprimer la SOP "${data.titre}" ? L'historique d'exécution sera également supprimé.`)) return;
    try {
      await window.SB.db.deleteSop(data.id);
      onBack();
    } catch (err) { window.notify('Erreur : ' + err.message, 'error'); }
  };
