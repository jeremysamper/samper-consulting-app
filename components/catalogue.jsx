...
const Catalogue = ({ user, etablissement }) => {
  ...
  React.useEffect(() => {
    ...
    const reload = async () => {
      try {
        const [ps, fs] = await Promise.all([
          ...
          window.SB.db.listFournisseurs(etabId),
        ]);
        if (!mounted) return;
        setProduits(ps);
        setFournisseurs(fs);
    ...
    reload();
    unsubs.push(window.SB.realtime.subscribe('produits', reload));
    unsubs.push(window.SB.realtime.subscribe('fournisseurs', reload));
    unsubs.push(window.SB.realtime.subscribe('produit_fournisseurs', reload));
  ...
  const saveFourn = async (f) => {
    if (!window.SB) return;
    try {
      await window.SB.db.upsertFournisseur({ ...f, etablissementId: etabId });
      setShowFournForm(false);
    } catch (err) { window.notify('Erreur : ' + err.message, 'error'); }
  };
  const deleteFourn = async (id) => {
    if (!window.confirm('Supprimer ce fournisseur ? Les produits associés perdront le lien.')) return;
    try { await window.SB.db.deleteFournisseur(id); } catch (err) { window.notify('Erreur : ' + err.message, 'error'); }
  };
  ...
  const saveProd = async (p) => {
    if (!window.SB) return;
    try {
      await window.SB.db.upsertProduit({ ...p, etablissementId: etabId });
      setShowProdForm(false);
    } catch (err) { window.notify('Erreur : ' + err.message, 'error'); }
  };
  const deleteProd = async (id) => {
    if (!window.confirm('Supprimer ce produit du catalogue ?')) return;
    try { await window.SB.db.deleteProduit(id); } catch (err) { window.notify('Erreur : ' + err.message, 'error'); }
  };
  ...
  const handleImportExcel = async (e) => {
    ...
    try {
      ...
      window.notify('Erreur import : ' + err.message, 'error');
    ...
    try {
      ...
      for (const p of toImport) {
        try {
          ...
          await window.SB.db.upsertProduit(payload);
          saved++;
        } catch (err) {
          errors.push(p.nom);
      ...
      window.notify(`✓ Import terminé : ${saved} produit(s)${errors.length ? `\n⚠ ${errors.length} en erreur (voir console).` : '.'}`, 'success');
    } finally {
      setImporting(false);
    }
  };
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                      ...
                      <select
                        ...
                        onChange={async e => {
                          ...
                          if (!fourn) return;
                          if (!window.confirm(`Attribuer "${fourn.nom}" à ${selected.size} produit${selected.size > 1 ? 's' : ''} sélectionné${selected.size > 1 ? 's' : ''} ?`)) return;
                          let ok = 0;
                          for (const prodId of selected) {
                            const prod = produits.find(p => p.id === prodId);
                            if (!prod) continue;
                            try {
                              await window.SB.db.upsertProduit({ ...prod, fournisseurId, etablissementId: etabId });
                              ok++;
                            } catch (err) { console.error(err); }
                          }
                          setSelected(new Set());
                          alert(`✓ ${ok} produit${ok > 1 ? 's' : ''} mis à jour.`);
                        }}
                      >
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          for (const prodId of selected) {
                            try {
                              await window.SB.db.deleteProduit(prodId);
                              ok++;
                            } catch (err) {
                          ...
                          setSelected(new Set());
                          if (errors > 0) {
                            window.notify(`${ok} produit(s) supprimé(s), ${errors} en erreur (voir console).`, 'warning');
                          } else {
                            window.notify(`✓ ${ok} produit${ok > 1 ? 's' : ''} supprimé${ok > 1 ? 's' : ''}.`, 'success');
                          }
                        }}
                          ...
                          {canWrite ? (
                            <select
                              ...
                              onChange={async e => {
                                ...
                                try {
                                  await window.SB.db.upsertProduit({ ...p, fournisseurId: fId, etablissementId: etabId });
                                } catch (err) { window.notify('Erreur : ' + err.message, 'error'); }
                              }}
...
const ProduitForm = ({ prod, fournisseurs, etabId, onSave, onClose }) => {
  ...
  const addPF = async () => {
    if (!newPF.fournisseurId || !newPF.prixAchat) { alert('Sélectionnez un fournisseur et renseignez un prix.'); return; }
    const pf = { ...newPF, produitId: prod?.id };
    if (prod?.id && window.SB) {
      try {
        await window.SB.db.upsertProduitFournisseur({ ...pf, produitId: prod.id });
