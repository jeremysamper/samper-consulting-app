...
const Documents = ({ user, etablissement }) => {
  ...
  const createFolder = async () => {
    ...
    try {
      ...
      window.notify('Erreur création dossier : ' + err.message, 'error');
    }
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        window.notify(`Erreur upload "${file.name}" : ${err.message}`, 'error');
        failed++;
      }
    }
    setUploading(false);
    ...
    if (success > 0 && failed === 0) {
      ...
      window.notify(`${success} fichier(s) uploadé(s), ${failed} échec(s).`, 'error');
    }
  };
  ...
  const openFile = async (doc) => {
    ...
    try {
      ...
      window.notify('Erreur ouverture fichier : ' + err.message, 'error');
    }
  };
  ...
  const downloadFile = async (doc) => {
    ...
    try {
      ...
      window.notify('Erreur téléchargement : ' + err.message, 'error');
    }
  };

  const moveDoc = async (targetParentId) => {
    if (!moving || !window.SB) return;
    // Éviter de déplacer un dossier dans lui-même ou dans un de ses descendants
    if (targetParentId === moving.id) { window.notify('Impossible de déplacer un dossier dans lui-même.', 'error'); return; }
    try {
      await window.SB.db.moveDocument(moving.id, targetParentId);
      setMoving(null);
    } catch (err) {
      window.notify('Erreur déplacement : ' + err.message, 'error');
    }
  };
  ...
  const confirmRename = async () => {
    ...
    try {
      ...
      window.notify('Erreur renommage : ' + err.message, 'error');
    }
  };
          ...
          >
            ...
            >
              ...
              <button
                ...
                onClick={(e) => { e.stopPropagation(); window.open(previewing.url, '_blank'); }}
