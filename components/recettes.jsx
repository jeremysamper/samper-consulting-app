...
const Recettes = ({ user, etablissement }) => {
  ...
  React.useEffect(() => {
    ...
    const refreshPlats = async () => {
      try { const p = await window.SB.db.listPlats(etabId); if (mounted) setPlats(p); } catch(e) {}
    };
    unsubRec = window.SB.realtime.subscribe('recettes', refreshRec);
    unsubCart = window.SB.realtime.subscribe('cartes', refreshCart);
    unsubPlats = window.SB.realtime.subscribe('plats', refreshPlats);
    unsubPR = window.SB.realtime.subscribe('plat_recettes', refreshPlats);
